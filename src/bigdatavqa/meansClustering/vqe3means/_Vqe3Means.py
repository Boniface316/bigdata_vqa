import numpy as np
from meansClustering.coresetUtils import get_bestB, Algorithm2
from meansClustering.vqeUtils import data_to_graph, best_clusters, cluster_cost_whole_set, approximate_n_trials, random_n_trials
...
class Vqe_3_Means:
    ...
    def set_best_cluster_centres_by_brute_force(self, weights=False):
        ...
        '''
        if weights:
            self.best_coreset_centres = best_clusters(self.coreset_graph, self.data, weights=self.weights)
            return
        
        self.best_random_sample_centres = best_clusters(self.random_sample_graph, self.data)
    ...
    def get_vqe_simulation_results(self, depth, num_runs,  weights=False):
        if weights:
            return approximate_n_trials(self.coreset_graph, self.data, self.coreset, self.weights, depth, num_runs)
        
        return random_n_trials(self.random_sample_graph, self.data, self.random_sample, num_runs, depth)
